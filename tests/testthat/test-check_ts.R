...
test_that("check_ts", {
  expect_equal(check_ts(ts_y_nvalid, ts_d_nvalid, ts_w_nvalid, valid_min), ts_expect_nvalid)
  expect_equal(check_ts(ts_y_na, ts_d_na, ts_w_na, valid_min, w_min = 0.2), ts_expect_na)
  expect_equal(check_ts(ts_y_spk, ts_d_spk, ts_w_spk, valid_min, w_min = 0.2, y_min = y_min, y_max = y_max,
                        approx_spike = T), ts_expect_spk)
  expect_equal(check_ts(ts_y_rg, ts_d_rg, ts_w_rg, valid_min, w_min = 0.2, y_min = y_min, y_max = y_max,
                        approx_spike = F), ts_expect_rg)
})
